...
- (void)setMapType:(YFTMKMapType)type
{
    mapType = type;
...
- (void)setRegion:(YFTMKCoordinateRegion)region
{
...
{
    for (id<YFTMKOverlay>overlay in someOverlays)
    {
...
- (void)exchangeOverlayAtIndex:(NSUInteger)index1 withOverlayAtIndex:(NSUInteger)index2
{
    ...
    id < YFTMKOverlay > overlay1 = [overlays objectAtIndex: index1];
    id < YFTMKOverlay > overlay2 = [overlays objectAtIndex: index2];
...
- (void)insertOverlay:(id < YFTMKOverlay >)overlay atIndex:(NSUInteger)index
{
    ...
    YFTMKOverlayView *overlayView = nil;
...
- (YFTMKOverlayView *)viewForOverlay:(id < YFTMKOverlay >)overlay
{
    if (![overlays containsObject:overlay])
        return nil;
    return (YFTMKOverlayView *)[overlayViews objectForKey: overlay];
}
...
{
    for (id<YFTMKAnnotation>annotation in someAnnotations)
    {
...
- (YFTMKAnnotationView *)viewForAnnotation:(id < YFTMKAnnotation >)annotation
{
    if (![annotations containsObject:annotation])
        return nil;
    return (YFTMKAnnotationView *)[annotationViews objectForKey: annotation];
}
...
- (void)selectAnnotation:(id < YFTMKAnnotation >)annotation animated:(BOOL)animated
{
    ...
    YFTMKAnnotationView *annotationView = (id)[annotationViews objectForKey: annotation];
...
- (void)setSelectedAnnotations:(NSArray *)someAnnotations
{
    ...
    for (id <YFTMKAnnotation> annotation in oldSelectedAnnotations)
    {
...
- (void)webView:(WebView *)sender didClearWindowObject:(WebScriptObject *)windowScriptObject forFrame:(WebFrame *)frame
{
    ...
    [windowScriptObject setValue:self forKey:@"YFTMKMapView"];
}
