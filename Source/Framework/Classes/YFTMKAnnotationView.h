...
enum {
    YFTMKAnnotationViewDragStateNone = 0,      // View is at rest, sitting on the map.
    YFTMKAnnotationViewDragStateStarting,      // View is beginning to drag (e.g. pin lift)
    YFTMKAnnotationViewDragStateDragging,      // View is dragging ("lift" animations are complete)
    YFTMKAnnotationViewDragStateCanceling,     // View was not dragged and should return to it's starting position (e.g. pin drop)
    YFTMKAnnotationViewDragStateEnding         // View was dragged, new coordinate is set and view should return to resting position (e.g. pin drop)
};

typedef NSUInteger YFTMKAnnotationViewDragState;

@protocol YFTMKAnnotation;


@interface YFTMKAnnotationView : YFTMKView {
    NSString *reuseIdentifier;
    id <YFTMKAnnotation> annotation;
    NSString *imageUrl;
    ...
    BOOL canShowCallout;
    BOOL draggable;
    YFTMKAnnotationViewDragState dragState;
@private
    WebScriptObject *markerImage;
    WebScriptObject *latlngCenter;
}

- (id)initWithAnnotation:(id <YFTMKAnnotation>)annotation reuseIdentifier:(NSString *)reuseIdentifier;

@property (nonatomic, readonly) NSString *reuseIdentifier;
// Classes that override must call super.
- (void)prepareForReuse;

@property (nonatomic, strong) id <YFTMKAnnotation> annotation;
...
@property (nonatomic) YFTMKAnnotationViewDragState dragState;
